...
const CHANNELS = [
  ...
  },
  {
    ...
    subscribers: '1.44M',
    works: [
      { videoUrl: 'https://youtu.be/5iecaGW8mVw?si=f8rfepjJAwsPzCAx' }
    ],
  },
  {
  ...
  },
  {
    ...
    subscribers: '4.98M',
    works: [
      { videoUrl: 'https://youtu.be/f6G1Mag1px8?si=7QT_svaQaW-hNgQu' }
    ],
  },
  {
...
function ChannelCard({ channel }) {
  ...
  const validWorks = channel.works.filter(
    work => work.videoUrl || work.label
  )
          ...
          {validWorks.map((work, i) => {
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="cc-work-link"
                title={work.label ?? 'Watch video'}
              >
                {thumb ? (
                  <img
                    src={thumb}
                    alt={work.label ?? `Work ${i + 1}`}
                    className="cc-thumb"
                  />
                ) : (
                  <div className="cc-thumb cc-thumb-placeholder">
                    ▶
                  </div>
                )}
  ...
  return (
    <section className="cc-section">
      ...
      <p className="cc-subtitle">
        Thumbnails for your favorite minecraft videos!
      </p>

      <div className="cc-wrapper">
        <button
          className={`cc-arrow cc-arrow-left ${
            !canLeft ? 'cc-arrow-hidden' : ''
          }`}
          onClick={() => scroll(-1)}
          aria-label="Previous"
        >
