...
export default function AdminPage() {
  ...
  async function saveChannels(list) {
    ...
    const cleaned = (list ?? []).map(ch => ({
      ...
      works: (ch.works ?? []).map(w => {
        const out = {}
        if (w && typeof w === 'object') {
          if (w.videoUrl) out.videoUrl = w.videoUrl
    ...
    try {
      await setDoc(doc(db, 'channels', 'list'), { channels: cleaned })
      setChannelsSaved(true)
      setTimeout(() => setChannelsSaved(false), 2000)
      setChannelsError('')
      return true
    } catch (err) {
      console.error('[admin] saveChannels failed', err)
  ...
  function addVideoToChannel(chIndex) {
    ...
    const updated = channels.map((ch, i) => {
      ...
      return { ...ch, works: [...(ch.works ?? []), workObj] }
    })
    setChannels(updated)
  ...
  async function addItem(field) {
    ...
    const prefixMap = { thumbnails: 'TN_', keyarts: 'KY_', profiles: 'PP_', promocional: 'WPP_' }
    const serviceKey = selectedService || ''
    const prefix = prefixMap[serviceKey] ?? ''

    const itemName = prefix ? `${prefix}${clientName}` : (
      serviceKey ? `${clientName} — ${(GALLERY_CATEGORY_LABELS[serviceKey] ?? serviceKey)}` : clientName
    )
  ...
  function removeClient(index) {
    setCommissions(prev => {
      ...
      // if removed client was selected, clear selection
      const removed = prev.clients[index]
          ...
          <div className="admin-page">
            ...
            <div className="admin-section">
              ...
              <div className="admin-table-wrapper">
                ...
                {(commissions.clients?.length ?? 0) > 10 && (
                  <div style={{ display: 'flex', justifyContent: 'center', marginTop: '0.5rem' }}>
                    <button className="admin-more-btn" onClick={() => setExpandedClients(prev => !prev)}>
                      {expandedClients ? 'Ver menos' : `Ver mais (${(commissions.clients?.length ?? 0) - 10})`}
                    </button>
                  </div>
                )}
              </div>
            ...
            <div className="admin-add-options" style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                <label style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.9rem' }}>Cliente:</span>
                  <select value={selectedClient} onChange={e => setSelectedClient(e.target.value)}>
                    <option value="">— selecionar cliente —</option>
                    {[...(commissions.clients ?? [])].sort((a,b) => (a.name || '').localeCompare(b.name || ''))
                      .map(c => (
                ...
                <label style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.9rem' }}>Serviço:</span>
                  <select value={selectedService} onChange={e => setSelectedService(e.target.value)}>
                    <option value="">— tipo de serviço —</option>
                    {Object.entries(GALLERY_CATEGORY_LABELS).map(([k, lab]) => (
                ...
                <label style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.9rem' }}>Etapa:</span>
                ...
                <button className="admin-add-stage-btn" onClick={() => addItem(selectedStage)}>Adicionar na etapa selecionada</button>
              </div>
                ...
                >
                  ...
                  <ul className="admin-list">
                    {(() => {
                      ...
                      const items = showAll ? all : all.slice(0, 10)
                      return items.map((item, i) => {
                        // when slicing, keep original index for removal accuracy
                        const originalIndex = showAll ? i : i
                        return (
                          <li
                            key={i}
                            draggable
                            onDragStart={() => onKanbanDragStart(field, originalIndex)}
                            className={`admin-card ${dragging?.field === field && dragging?.index === originalIndex ? 'dragging' : ''}`}
                          >
                            <span>⠿ {item}</span>
                            <button className="admin-remove" onClick={() => removeItem(field, originalIndex)}>✕</button>
                          </li>
                        )
                      })
                    })()}
                  </ul>
                  {field === 'concluidos' && (commissions.concluidos?.length ?? 0) > 10 && (
                    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '0.5rem' }}>
                      <button className="admin-more-btn" onClick={() => setExpandedCompleted(prev => !prev)}>
                        {expandedCompleted ? 'Ver menos' : `Ver mais (${(commissions.concluidos?.length ?? 0) - 10})`}
                      </button>
                    </div>
                  )}
                </div>
